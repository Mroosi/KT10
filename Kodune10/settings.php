...
<body>
    ...
    <form method="POST" action="?">
        ...
        <br/>
        <input type="color" name="bg" id="bg" value="<?php if (isset($_POST['bg'])) 
                                                        echo htmlspecialchars($_POST['bg']);  ?>" > 
        <label for="bg">Taustavärvus</label>
        <br/>
        <input type="color" name="tc" id="tc" value="<?php if (isset($_POST['tc'])) 
                                                        echo htmlspecialchars($_POST['tc']);  ?>"  > 
        <label for="tc">Tekstivärvus</label>
        <br/>
        <fieldset>
            <legend>Piirjoon</legend>
            <input type="number" min="0" max="20" step="1" name="bw" id="bw" value="<?php if (isset($_POST['bw'])) 
                                                        echo htmlspecialchars($_POST['bw']);  ?>" >
            <label>Piirjoone laius (0-20px)</label>
            <br/>
            <select name="bs">
                <?php if (isset($_POST['bs'])){
                     echo htmlspecialchars($_POST['bg']);
                } ?>
            ...
            <br/>
            <input type="color" name="bc" id="bc" value="<?php if (isset($_POST['bc'])) 
                                                        echo htmlspecialchars($_POST['bc']);  ?>"  > 
            <label for="bc">Piirjoone värvus</label>
            <br/>
            <input type="number" min="0" max="100" step="1" name="br" id="br" value="<?php if (isset($_POST['br'])) 
                                                            echo htmlspecialchars($_POST['br']);  ?>"  >
